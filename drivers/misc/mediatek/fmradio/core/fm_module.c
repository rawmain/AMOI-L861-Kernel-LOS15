...
#define FM_IOCTL_GET_AUDIO_INFO2 0xc004f530
#define FM_IOCTL_I2S_SETTING2 0xc004f521
...
static long fm_ops_ioctl(struct file *filp, fm_u32 cmd, unsigned long arg)
{
	...
	default:
	printk("FMDRV unknown command %s---orginal(0x%08x)---cmd(0x%08x)---arg(0x%08x)\n", current->comm,
		(fm_u32) FM_IOCTL_GET_AUDIO_INFO, cmd, (fm_u32) arg);
		ret = -EPERM;
	}
