...
//amoi lincg@20150127 begin
extern unsigned int UbVbat;
extern void get_UbVbat_value(void);
//amoi lincg@20150127 end
...
#endif
            //amoi begin
            if (bat_temperature_volt >= RBAT_PULL_UP_VOLT) {
                bat_temperature_volt = RBAT_PULL_UP_VOLT - 1;
            }
            //amoi end
...
/* 5. Logging */
    ...
    //amoi lincg begin
    bm_print(BM_LOG_FULL, "UbVbat_: %d \n",UbVbat);
    //amoi lincg end
...
void fgauge_initialization(void)
{
    ...
    //amoi lincg@20150127 begin   
    bm_print(BM_LOG_CRTI, "UbVbat1: %d ,gFG_voltage=%d\n",UbVbat,gFG_voltage);
    get_UbVbat_value();

	if((UbVbat > 0) && (abs(gFG_voltage - UbVbat) > 50))//这里的目的是防止HW 电压不准
	{
		bm_print(BM_LOG_CRTI," gFG_voltage = %d, UbVbat = %d\n",gFG_voltage, UbVbat);
		gFG_voltage=UbVbat;
	}
	bm_print(BM_LOG_CRTI, "UbVbat2: %d,gFG_voltage=%d\n",UbVbat,gFG_voltage);
	//amoi lincg@20150127 end
